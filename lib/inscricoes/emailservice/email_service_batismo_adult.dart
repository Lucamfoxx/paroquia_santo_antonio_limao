...
import 'package:intl/intl.dart'; // Importando para formatar a data
...
class EmailService {
  ...
  String _buildEmailBody(Map<String, String> formData) {
    String? dataBatismo = formData['dataBatismo'];

    // Formatar a data de "dataBatismo" se estiver presente
    String dataBatismoFormatada = 'Não especificada';
    if (dataBatismo != null && dataBatismo.isNotEmpty) {
      try {
        DateTime parsedDate = DateFormat('dd/MM/yyyy').parse(dataBatismo);
        dataBatismoFormatada =
            DateFormat('d \'de\' MMMM \'de\' y', 'pt_BR').format(parsedDate);
      } catch (e) {
        print('Erro ao formatar a data do batismo: $e');
      }
    }
    ...
    CEP: ${formData['cep']}
    Data do Batismo: $dataBatismoFormatada
